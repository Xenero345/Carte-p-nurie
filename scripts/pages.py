...
VOYELLES = "AEIOUÀÂÉÈÊËÎÏÔÙÛÜ"


def de_nom(nom):
    """« de » + nom de département, avec élision (de l'Ain, d'Eure-et-Loir, de la Sarthe...)."""
    return f"d'{nom}" if nom[0] in VOYELLES else f"de {nom}"
...
def construire_page(gabarit, dep, deps_releve, voisins, maj):
    ...
    if s:
        desc = (f"{fmt_pct(s['pct'])} % des stations-service {de_nom(nom)} sont en rupture partielle ou totale "
...
<meta property="og:image:alt" content="Carte de France des stations-service en rupture de carburant">
...
{{
  ...
  "offers": {{"@type": "Offer", "price": "0", "priceCurrency": "EUR"}}
    ...
    if s:
        chiffres = (f"<b>{fmt_pct(s['pct'])} %</b> des {s['all']} stations-service {de_nom(nom)} sont actuellement en rupture "
