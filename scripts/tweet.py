...
def main():
    ...
    ap.add_argument("--intent", action="store_true",
                    help="affiche le tweet + un lien qui ouvre X avec le texte pré-rempli (gratuit, rien n'est publié)")
    a = ap.parse_args()
    texte = composer()
    print(texte)
    print(f"\n({longueur_x(texte)}/280 caractères au comptage X)", file=sys.stderr)
    if a.intent:
        print("\nLien de publication : https://twitter.com/intent/tweet?text=" + urllib.parse.quote(texte, safe=""))
    elif not a.dry_run:
        publier(texte)
